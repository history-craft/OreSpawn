...
public class AntRobot extends EntityLiving
{
    ...
    public AntRobot(final World par1World) {
        ...
        this.tasks.addTask(1, (EntityAIBase)new EntityAIWatchClosest((EntityLiving)this, (Class)EntityPlayer.class, 12.0f));
    ...
    protected void updateAITasks() {
        ...
        if (this.owned == 0 && this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL) {
            ...
            if (e != null && this.worldObj.rand.nextInt(15) == 0) {
                ...
                if (e != null) {
                    if (this.getDistanceSqToEntity((Entity)e) < (6.0f + e.width / 2.0f) * (6.0f + e.width / 2.0f)) {
                        this.setAttacking(1);
    ...
    public void updateLegs() {
        ...
        for (i = 0, i = 0; i < 6; ++i) {
            ...
            n = i;
            ++footingticker[n];
            this.renderdata.realposx[i] = (float)(this.posX - this.renderdata.legoff[i] * Math.sin(Math.toRadians(MathHelper.wrapAngleTo180_double((double)(this.rotationYaw + 90.0f))) + this.renderdata.ymid[i]));
            this.renderdata.realposz[i] = (float)(this.posZ + this.renderdata.legoff[i] * Math.cos(Math.toRadians(MathHelper.wrapAngleTo180_double((double)(this.rotationYaw + 90.0f))) + this.renderdata.ymid[i]));
            ...
            dd *= 16.0f;
            da = (float)(Math.abs(this.renderdata.ycurrentangle[i] - (Math.toRadians(MathHelper.wrapAngleTo180_double((double)this.rotationYaw)) + this.renderdata.ymid[i])) % 6.283185307179586);
            ...
            for (dd = (float)(this.renderdata.ycurrentangle[i] - Math.toRadians(MathHelper.wrapAngleTo180_double((double)this.rotationYaw)) - 1.5707963267948966); dd > 3.141592653589793; dd -= (float)6.283185307179586) {}
    ...
    private void findNewFooting(final int i) {
        ...
        final float deffx;
        float fx = deffx = (float)(this.renderdata.realposx[i] - f / 2.0f * Math.sin(Math.toRadians(MathHelper.wrapAngleTo180_double((double)(this.rotationYaw + 90.0f))) + this.renderdata.ymid[i]));
        final float deffz;
        float fz = deffz = (float)(this.renderdata.realposz[i] + f / 2.0f * Math.cos(Math.toRadians(MathHelper.wrapAngleTo180_double((double)(this.rotationYaw + 90.0f))) + this.renderdata.ymid[i]));
        final float deffy;
        float fy = deffy = this.renderdata.realposy[i] - 1.0f;
        final float oldf = f;
        int span = 1;
        while (found == 0 && f > 2.5f) {
            fx = (float)(this.renderdata.realposx[i] - f * Math.sin(Math.toRadians(MathHelper.wrapAngleTo180_double((double)(this.rotationYaw + 90.0f))) + this.renderdata.ymid[i] - range));
            fz = (float)(this.renderdata.realposz[i] + f * Math.cos(Math.toRadians(MathHelper.wrapAngleTo180_double((double)(this.rotationYaw + 90.0f))) + this.renderdata.ymid[i] - range));
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.rand.nextInt(18) == 0) {
            this.worldObj.spawnParticle("flame", this.posX + dx, this.posY + 0.5, this.posZ + dz, (double)(dx2 / f + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 20.0f), (double)((this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 10.0f), (double)(dz2 / f + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 20.0f));
        }
        if (this.worldObj.rand.nextInt(7) == 0) {
            this.worldObj.spawnParticle("smoke", this.posX + dx, this.posY + 0.5, this.posZ + dz, (double)(dx2 / f + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 20.0f), (double)((this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 10.0f), (double)(dz2 / f + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 20.0f));
        }
        if (this.worldObj.rand.nextInt(16) == 0) {
            this.worldObj.spawnParticle("fireworksSpark", this.posX + dx, this.posY + 0.5, this.posZ + dz, (double)(dx2 / f + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 20.0f), (double)((this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 5.0f), (double)(dz2 / f + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) / 20.0f));
        }
    ...
    public boolean feetattackEntityAsMob(final Entity par1Entity) {
        ...
        if (par1Entity != null && par1Entity instanceof EntityLivingBase) {
            ...
            ret = par1Entity.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), OreSpawnMain.AntRobot_stats.attack / 10.0f);
    ...
    private EntityLivingBase findSomethingToAttack(final float distmul, final boolean dircheck) {
        ...
        final List var5 = this.worldObj.getEntitiesWithinAABB((Class)EntityLivingBase.class, this.boundingBox.expand(12.0 * distmul, 12.0, 12.0 * distmul));
        final Iterator var6 = var5.iterator();
        Entity var7 = null;
        EntityLivingBase var8 = null;
        while (var6.hasNext()) {
