...
public class Godzilla extends EntityMob
{
    ...
    public Godzilla(final World par1World) {
        ...
        this.tasks.addTask(3, (EntityAIBase)new EntityAIWatchClosest((EntityLiving)this, (Class)EntityLiving.class, 50.0f));
                ...
                for (int j = -xzrange; j <= xzrange; ++j) {
                    ...
                    if (this.isCrushable(bid)) {
                        ...
                        if (this.worldObj.rand.nextInt(15) == 1) {
                            this.dropItemRand(Item.getItemFromBlock(bid), 1);
                        }
                    }
                    else {
            ...
            for (int i = -xzrange; i <= xzrange; ++i) {
                for (int j = -xzrange; j <= xzrange; ++j) {
                    ...
                    if (this.isCrushable(bid)) {
                        ...
                        if (this.worldObj.rand.nextInt(15) == 1) {
                            this.dropItemRandAt(Item.getItemFromBlock(bid), 1, dx, dz);
                        }
                    }
                    else {
                        if (bid == Blocks.grass && this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing")) {
        ...
        if (this.worldObj.rand.nextInt(5 - this.large_unknown_detected) == 1) {
            ...
            if (e == null) {
                ...
                if (this.head_found == 0) {
                    final EntityLiving newent = (EntityLiving)spawnCreature(this.worldObj, "MobzillaHead", this.posX, this.posY + 20.0, this.posZ);
                }
            }
            if (e != null) {
                ...
                else if (this.MygetDistanceSqToEntity((Entity)e) < 300.0f + e.width / 2.0f * (e.width / 2.0f)) {
                    this.setAttacking(1);
    ...
    private boolean isVillagerTarget(final EntityLivingBase par1EntityLiving, final boolean par2) {
        return par1EntityLiving != null && par1EntityLiving != this && par1EntityLiving.isEntityAlive() && this.getEntitySenses().canSee((Entity)par1EntityLiving) && par1EntityLiving instanceof EntityVillager;
    }
    ...
    private EntityLivingBase doJumpDamage(final double X, final double Y, final double Z, final double dist, final double damage, final int knock) {
        ...
        while (var6.hasNext()) {
            ...
            this.worldObj.playSoundAtEntity((Entity)var8, "random.explode", 0.85f, 1.0f + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.5f);
            if (knock == 0) {
                continue;
            }
            final double ks = 3.5;
    ...
    private EntityLivingBase findSomethingToAttack() {
        ...
        int vf = 0;
        var5 = this.worldObj.getEntitiesWithinAABB((Class)EntityLivingBase.class, this.boundingBox.expand(64.0, 40.0, 64.0));
        if (var5 == null) {
            return null;
        }
        Collections.sort((List<Object>)var5, this.TargetSorter);
    ...
    public boolean getCanSpawnHere() {
        ...
        target = (Godzilla)this.worldObj.findNearestEntityWithinAABB((Class)Godzilla.class, this.boundingBox.expand(64.0, 16.0, 64.0), (Entity)this);
        if (target != null) {
            return false;
        }
        if (!this.worldObj.isRemote) {
