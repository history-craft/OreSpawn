...
public class Whale extends EntityAnimal
{
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.isRemote && this.spray > 0) {
            for (int i = 0; i < 20; ++i) {
                ...
                if (i < 10) {
                    this.worldObj.spawnParticle("bubble", this.posX + dx, this.posY + 1.0 + d, this.posZ + dz, Math.cos(dir) * this.worldObj.rand.nextFloat() / 4.0, (double)(this.worldObj.rand.nextFloat() * 2.0f), Math.sin(dir) * this.worldObj.rand.nextFloat() / 4.0);
                }
                else {
                    this.worldObj.spawnParticle("splash", this.posX + dx, this.posY + 1.0 + d, this.posZ + dz, Math.cos(dir) * this.worldObj.rand.nextFloat() / 4.0, (double)(this.worldObj.rand.nextFloat() * 2.0f), Math.sin(dir) * this.worldObj.rand.nextFloat() / 4.0);
                }
            }
            --this.spray;
        }
    ...
    private int findBuddies() {
        final List var5 = this.worldObj.getEntitiesWithinAABB((Class)Whale.class, this.boundingBox.expand(32.0, 8.0, 32.0));
        return var5.size();
    }
